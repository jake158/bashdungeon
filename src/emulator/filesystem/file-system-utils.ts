...
function permsToOctal(perms: string): string {
    ...
    for (let i = 1; i < perms.length; i += 3) {
        let value = 0;
        value += permissionBits[perms[i] as PermissionChar];
        value += permissionBits[perms[i + 1] as PermissionChar];
        value += permissionBits[perms[i + 2] as PermissionChar];
        octal += value.toString(8);
    }
    return octal;
}

function octalToPerms(octal: string, isDirectory: boolean = false): string {
    const permissionChars: Record<number, string> = { 4: 'r', 2: 'w', 1: 'x', 0: '-' };
...
export function applyUmask(permissions: string, umask: string): string {
    const permOctal = permsToOctal(permissions);
    const result = parseInt(permOctal) - parseInt(umask);
    return octalToPerms(result.toString().padStart(3, '0'), permissions[0] === 'd');
}

export function parseChmodString(string: string, currentPermissions: string): string {
...
function _processMode(mode: string, permissions: string[]): void {
...
function _applyPermissions(groups: string, operator: string, perms: string, permissions: string[]): void {
    ...
    const currentGroupPerms: Record<string, string[]> = {
...
function _applyGroupPerms(indexList: number[], operator: string, permSet: string, permissions: string[]): void {
...
function _applyIndividualPerms(indexList: number[], operator: string, permSet: string, permissions: string[]): void {
    ...
    for (const perm of permSet) {
        const posIndeces = permMap[perm];
        if (!posIndeces) continue;
